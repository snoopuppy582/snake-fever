(() => {
    ...
    const stick = new SFStick(byId('stick'), value => send('turn', value));
    let controlMode = 'stick';
    try { if (localStorage.getItem('snake-fever.controls') === 'buttons') controlMode = 'buttons'; } catch (_) {}
    const setControlMode = value => {
        controlMode = value;
        document.body.dataset.controls = value;
        document.querySelectorAll('[name=control-mode]').forEach(input => { input.checked = input.value === value; });
        window.sfClear();
        try { localStorage.setItem('snake-fever.controls', value); } catch (_) {}
    };
    const clearPressed = () => {
        pointers.clear();
        stick.reset();
    ...
    window.addEventListener('pointermove', event => {
        ...
        if (!previous?.dataset.direction) return;
        const button = document.elementsFromPoint(event.clientX, event.clientY)
            .find(element => element.matches?.('#dpad [data-direction]'));
        if (!button || button === previous || button.disabled) return;
        pointers.set(event.pointerId, button);
        if (![...pointers.values()].includes(previous)) previous.classList.remove('pressed');
        button.classList.add('pressed');
        send('turn', button.dataset.direction);
    });
    ...
    document.querySelectorAll('[name=control-mode]').forEach(input => {
        input.addEventListener('change', () => { if (input.checked) setControlMode(input.value); });
    });
    ...
    window.sfRender = raw => {
        ...
        stick.setEnabled(window.sfTouch && controlMode === 'stick' && !s.panel && ['ready','playing'].includes(s.status) && s.countdown <= 0);
