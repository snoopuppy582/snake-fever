(() => {
    ...
    window.sfRender = raw => {
        ...
        byId('fever-label').textContent = s.fever ? (s.left <= 2 ? 'ENDING' : 'FEVER TIME') : s.ready ? 'READY' : 'RECHARGING';
        ...
        byId('fever-button').disabled = !s.ready || s.status !== 'playing' || !!s.panel || s.countdown > 0;
